...
- 允许显示：命中后允许邮件在访客链接中显示。
- 隐藏 / 排除：命中后隐藏邮件；同一分享链接内隐藏 / 排除动作优先级高于允许显示动作。

规则管理页提供可视化条件组编辑器：

- 条件组支持 `AND` / `OR` 组合和 `NOT` 反向匹配。
- 条件支持 `contains`、`exact`、`startsWith`、`endsWith`、`regex`。
- 条件和分组支持基于 Pointer Events 的卡片拖拽排序/移动，拖动时显示 ghost preview、插入占位和可放置区域高亮；复制、上移/下移、重置按钮保留为键盘友好的兜底。
- “批量生成条件”可从多关键词快速生成规则树；“高级表达式 JSON 预览 / 导入”用于复制、审查或导入表达式。

分享链接创建时选择一个或多个规则，必须至少包含一个启用的允许显示规则；可以额外绑定隐藏 / 排除规则用于排除垃圾邮件、营销邮件或误命中邮件。允许显示规则组和隐藏 / 排除规则组都可选择 `AND` / `OR` 组合逻辑，默认均为 `OR` 以兼容旧链接。访客 API 每次请求都会计算 `now - 30 minutes` 到当前时刻的窗口，所以页面轮询时能看到新收到的匹配邮件，但不会继续展示超过窗口的旧邮件。
