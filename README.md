...
规则可以使用多关键词，也可以填写高级表达式 JSON，支持 `and` / `or` / `not` 嵌套条件；条件操作符支持 `contains`、`exact`、`startsWith`、`endsWith`、`regex`。分享链接创建时选择一个或多个规则，必须至少包含一个启用的白名单规则；可以额外绑定黑名单规则用于排除垃圾邮件、营销邮件或误命中邮件。访客 API 每次请求都会计算 `now - 30 minutes` 到当前时刻的窗口，所以页面轮询时能看到新收到的匹配邮件，但不会继续展示超过窗口的旧邮件。
