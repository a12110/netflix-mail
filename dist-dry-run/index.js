...
function attachmentSize(attachment) {
  ...
  if (typeof content === "string") {
    return attachment.encoding === "base64" ? Math.floor(content.length * 3 / 4) : utf8ByteLength(content);
  }
  return null;
}
...
async function createFirstAdmin(c) {
  ...
  if (!body?.setupToken || !body.username || !body.password) {
    return badRequest(c, "setupToken, username and password are required.");
  }
  if (!timingSafeEqual(body.setupToken, c.env.ADMIN_SETUP_TOKEN)) {
    return forbidden(c, "Invalid setup token.");
  }
  if (body.password.length < 10) {
    return badRequest(c, "Password must be at least 10 characters.");
  }
  const id = await createAdmin(c.env.DB, body.username.trim(), body.password);
